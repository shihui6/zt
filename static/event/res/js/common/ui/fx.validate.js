	...
	M.ui.define('validate',function(){
		...
		Validate.prototype={
			...
			init:function(){
				...
				this.ops.type=M.trim(this.ops.container.attr('valtype')).split('|');
				this.ops.isMust=/true/g.test(M.trim(this.ops.container.attr('unMust')))?false:true;
				this.status=false;
				this.msg=this.ops.container.attr('error');
				...
				this.status?this.ops.success.call(this.ops.container):this.ops.error.call(this.ops.container,this.msg);
				return this.status;	
			},
			test:function(type){
				return this.ops.isMust?this.exec(type):this.ops.container.val().length?this.exec(type):true;
			},
			exec:function(type){
				var reg=this.regexp(this.ops.number),obj={IDcard:'checkIDCard',card:'cardsValid',againPassword:'checkPassword'};
				return obj.hasOwnProperty(type)?this[obj[type]](reg[type],reg):function(that){
					return reg[type].test(that.ops.container.val());
				}(this);	
			},
			regexp:function(n){
				return {
					...
					againPassword:/^[!"#$%&'()*+,\-.:;<=>?@\[\\\]^_`{|}~A-Za-z0-9]{6,20}$/,
					trueName:/^[?!$%~_#`.·@^\u4e00-\u9fa5]+$/,
			...
			cardsValid:function(){
				var sum=0,pan=this.ops.container.val().toString();
				if(pan.length>0){
			...
			checkPassword:function(type,value){
				value=this.ops.container[0].value;
				if(type.test(value)){
					if(this.ops.container[0].value==document.getElementById('checkPassword').value){
						 return true;
					}else{
						 this.msg='两次密码不一致';
						 return false;	
			...
			checkIDCard:function(type,reg){
				 var value=this.ops.container.val(),l=value.length,re=type[l];
				 if(re==undefined){
					 this.msg='身份证号码位数不正确';
					 return false;
				 };
