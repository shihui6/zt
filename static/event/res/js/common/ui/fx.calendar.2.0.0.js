		...
		Calendar.prototype={
			...
			create:function(){
				...
				if(this.ops.time.enabled||this.ops.tool.clear||this.ops.tool.today){
					...
					if(this.ops.time.enabled){
						this.time=this.creatElement('mt-calendar-time');
						this.tip=M(M.creatlabel('span'),{
							'class':'mt-calendar-time-tip'.concat(String.fromCharCode(32)).concat(this.ops.number==1?'y':'x'),
							html:this.renderTip()
						});	
				...
				for(var i=0;i<name.length;i++){
					...
					if(!this[name[i]].scroll){
						this[name[i]].scroll=M.ui.scroll.init({
							scrollbar:{
								className:'mt-calendar-time-scroll',
								style:{
									marginLeft:0,
									marginRight:0,
									marginTop:5,
			...
			creatTime:function(fn){
				...
				for(var i=0,name=this.key.slice(8,11),number={hour:24,minute:60,second:60};i<name.length;i++){
					...
					this[name[i]].main=this.creatElement('mt-calendar-time-item');
					this[name[i]].text=M(M.creatlabel('em'),{
						'class':'number',
						html:M.formatDate(data[name[i]])
					});
					this[name[i]].inner=this.creatElement('mt-calendar-time-box'.concat(String.fromCharCode(32)).concat(name[i]));
			...
			quickYear:function(e){
				...
				that.addPanel(list,'_'.concat(that.key[5]),M.template.resolve(that.renderTitle(1),{value:list._year.start+'-'+(list._year.start+11)}),data.arr,2,function(ele){
					...
					if(!ele.prev){
						ele.prev=that.creatElement('mt-calendar-fast-btn prev');
						ele.main.append(ele.prev);
					};
					if(!ele.next){
						ele.next=that.creatElement('mt-calendar-fast-btn next');
			...
			getPanel:function(){
				var box=this.creatElement('mt-calendar-fast'),
					inner=this.creatElement('mt-calendar-inner');
					panel=this.creatElement('mt-calendar-panel');
				inner.append(panel);
				box.append(inner);
				return {
					box:box,
			...
			creatItem:function(){
				...
				for(var i=0,str='';i<this.ops.number;i++){
					...
					this.creatInner(i);
					this.list[i].group=this.creatElement('mt-calendar-item').append(this.list[i].head).append(this.list[i].body);	
			...
			creatInner:function(index){
				...
				this.list[index].body=this.creatElement('mt-calendar-body');
			...
			renderDay:function(){
				return M.renderHTML({
					proto:{
						'class':'mt-calendar-day'
					},
					html:M.renderHTML({
						proto:{
							'class':'mt-calendar-overs'
						},
						html:'{{month}}'
					})+M.renderHTML({
						name:'ul',
						proto:{
							'class':'mt-calendar-number'
						},
						html:'{{list}}'
					})
				});
			...
			creatSeparator:function(){
				return M(M.creatlabel('span'),{
					'class':'mt-calendar-time-separator',
					html:'：'
				});		
			},
			creatWarp:function(month,list){
				return M(M.creatlabel(),{
					'class':'mt-calendar-warp',
					html:M.renderHTML({
						proto:{
							'class':'mt-calendar-main'
						},
			...
			creatHead:function(name){
				return M(M.creatlabel(),{
					'class':'mt-calendar-head'.concat(String.fromCharCode(32)).concat(name)
				});	
			},
			...
			creatMonthButton:function(type){
				return M(M.creatlabel(),{
					'class':'mt-calendar-month-btn '+{1:'prev',2:'next'}[type],
