	...
	M.ui.define('waiting',function(){
		...
		Waiting.prototype={
			...
			creatNode:function(args){
				var icon={'true':M.Font.check_fill_lighter,'false':M.Font.close_fill,'tip':M.Font.error_fill_circle}[this.ops.status.toString()];
				this.container=M(M.creatlabel(),{
					...
					css:{
						position:'absolute',
						background:'none',
						marginLeft:this.ops.offsetLeft,
						marginTop:this.ops.offsetTop,	
						zIndex:M.now()
					}	
				});
				this.content=M(M.creatlabel(),{
					'class':'mt-waiting-content '+this.ops.status.toString(),
					css:{
				...
				if(icon){
					...
					M.ui.loadImage.init({
						...
						onComplete:function(that){
							that.content.css({
								background:'url('+M.getNormalPath(that.ops.src.inner)+') repeat'
							}).html(M.renderHTML({
								name:'img',
								proto:{
									src:this[0],
			...
			resolve:function(icon,that,args){
				...
				if(icon){
					this.content.css({
						...
						marginTop:-0.5*height-30
					}).animate({marginTop:-0.5*height,opacity:1},300,function(){
						that.dismiss(args,1,-0.5*height+30);		
					});	
				}else{
					this.dismiss(args,0);	
				};	
		...
		return {
			defaults:{
				...
				hide:true,
				src:{
					inner:'base/piel_black_1_1_alpha_2.png',
