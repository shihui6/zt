	...
	M.ui.define('scroll',function(){
		...
		Scroll.prototype={
			...
			creat:function(args){
				...
				this.ops.resize&&M(window).bind('resize',{context:this,args:args},this.reset);
			...
			scrollTo:function(value,args,time,ease){
				if(M.isNumeric(value)){
					this.ops.offset=value;
					this.setBar();	
					this.setContent(this,args,time||1,ease);
				};
			},
			scrollTop:function(args,time,ease){
				this.scrollTo(0,args,time,ease);
			},
			...
			clear:function(){
				...
				this.ops.container.unbind('mousewheel');
				this.ops.resize&&M(window).unbind('resize',this.reset);
			},
			...
			setContent:function(that,args,time,ease){
				this.ops.content.stop().animate({
					x:{left:-this.ops.offset},
					y:{top:-this.ops.offset}
				}[this.ops.direction],time||300,ease||'linear',function(){
