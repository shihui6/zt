	...
	M.ui.define('cityQuery',function(){
		...
		cityQuery.prototype={
			...
			addEvent:function(name,type,data,args){
				this[name].find(document.getElementsByClassName('mt-pop-piel')).bind('click',{context:this,type:type,list:data,args:args},this.select);
			...
			render:function(){
				return '<li><a href="javascript:;" title="{{name}}" uid="{{value}}" class="mt-pop-piel">{{name}}</a></li>';	
			},
			creatTab:function(){
			...
			creatContent:function(){
				return M(M.creatlabel(),{
					'class':'mt-pop-content pad-10'
				});	
			},
			creatClose:function(){
				return M(M.creatlabel(),{
					'class':'iconfont mt-pop-close',
