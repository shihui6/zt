		...
		Status.prototype={
			...
			creat:function(args){
				...
				this.container=M(M.creatlabel(),{
					'class':'mt-master-container mt-status-master',
					id:this.ops.id||'mt-status-'+M.now(),
					css:{
						opacity:0,
			...
			resolve:function(args){
				var that=this,width=Math.max(400,this.content.outerWidth(true)),height=this.content.outerHeight(true);
			...
			renderBody:function(){
				return M(M.creatlabel(),{
					'class':'mt-master-body clear align-center pad-bottom-20',
					html:M.renderHTML({
						proto:{
			...
			renderIcon:function(){
				return M.renderHTML({
					name:'em',
					proto:{
						'class':'iconfont mt-status-icon middle '+this.ops.status.toString()
					},
					html:this.icon
				});	
			},
